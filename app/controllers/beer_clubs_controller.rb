class BeerClubsController < ApplicationController
  ...
  def show
    if current_user
      if @beer_club.memberships.pluck(:user_id).include? current_user.id
        @membership=@beer_club.memberships.select{|m| m.user_id==current_user.id}.first
      else
        @membership = Membership.new user:current_user, beer_club_id:@beer_club.id
      end
    end
  end
